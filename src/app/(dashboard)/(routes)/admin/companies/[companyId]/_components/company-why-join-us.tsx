...
interface CompanyWhyJoinUsFormProps {
  initialData: Company;
  companyId: string;
}
...
export const CompanyWhyJoinUsForm = ({
  initialData,
  companyId,
}: CompanyWhyJoinUsFormProps) => {
  ...
  const handlePromptGenerator = async () => {
    try {
      setIsPrompting(true);
      const customPrompt = `Create a persuasive "Why join us" content piece for ${companyName}. 
      Highlight the unique benefits and experiences offered, emphasizing 
      the value proposition exclusive content, personalized recommendations, and community features. 
      Tailor the content to attract potential users and showcase how ${companyName} 
      stands out among other platforms.`;
  ...
  const oncopy = () => {
    ...
    toast.success("Copy whyJoinUs To Clipboard");
  };
  ...
  return (
    <div className="rounded-md border bg-neutral-100 p-4">
      ...
      {!isEditing && (
        ...
        >
          {!initialData.whyJoinUs && "No whyJoinUs"}
      ...
      {isEditing && (
        <>
          ...
          <p className="my-2 text-right text-xs text-neutral-500">
            Note: Type the Company name here to genereate the whyJoinUs content.
          </p>
